...
if len(sys.argv) != 5:
    print("usage: python3 varying_delay.py <bottleneck-delay> <bottleneck-bandwidth> <edge-delay> <edge-bandwidth>")
    sys.exit(1)

# Assigning number of nodes on either sides of the dumbbell according to input
num_of_left_nodes = 3
num_of_right_nodes = 3

script, bottleneck_delay, bottleneck_bandwidth, edge_delay, edge_bandwidth = sys.argv
...
# Setting up the attributes of the connections between
# the two routers
left_router_connection.set_attributes(bottleneck_bandwidth, bottleneck_delay,"pfifo")
right_router_connection.set_attributes(bottleneck_bandwidth, bottleneck_delay,"pfifo")
